...
def main():
    ...
    if st.session_state.get('data_loaded', False) and st.session_state.get('df_long') is not None:
        ...
        with st.sidebar.expander("Opciones de Modelo Digital de Elevación (DEM)", expanded=True):
            ...
            if uploaded_dem_file or st.session_state.get('dem_raster') is not None:
                if f'original_{Config.ALTITUDE_COL}' not in st.session_state:
                    st.session_state[f'original_{Config.ALTITUDE_COL}'] = st.session_state.gdf_stations.get(Config.ALTITUDE_COL, None).copy()
                ...
                if st.session_state.get(f'original_{Config.ALTITUDE_COL}') is not None:
        ...
        if df_anual_melted.empty or df_monthly_filtered.empty or gdf_filtered.empty:
            with tabs[0]:
                st.warning("No hay datos disponibles para los filtros aplicados. Ajuste la selección.")
            return
        ...
        with tabs[3]:
            display_advanced_maps_tab(gdf_filtered, stations_for_analysis, df_anual_melted, df_monthly_filtered)
        with tabs[4]:
        ...
        with tabs[6]:
            display_stats_tab(st.session_state.df_long, df_anual_melted, df_monthly_filtered, stations_for_analysis, gdf_filtered)
        with tabs[7]:
